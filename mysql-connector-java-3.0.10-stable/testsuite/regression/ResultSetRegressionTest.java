...
public class ResultSetRegressionTest extends BaseTestCase {
    ...
    /**
     * Tests fix for BUG#1630 (not updatable exception turning into NPE
     * on second updateFoo() method call.
     *
    ...
    /**
     * Checks fix for BUG#1592 -- cross-database updatable result sets
    ...
    public void testFixForBug1592() throws Exception {
    	if (versionMeetsMinimum(4, 1)) {
			...
			try {
				...
				if (message != null && message.indexOf("Access denied") != -1) {
					System.err.println("WARN: Can't complete testFixForBug1592(), access to" +
    ...
    public void testFixForBug2006() throws Exception {
    	try {
    		...
    		this.stmt.executeUpdate("CREATE TABLE testFixForBug2006_1 (key_field INT NOT NULL)");
    		this.stmt.executeUpdate("CREATE TABLE testFixForBug2006_2 (key_field INT NULL)");
    		this.stmt.executeUpdate("INSERT INTO testFixForBug2006_1 VALUES (1)");
    		
    		this.rs = this.stmt.executeQuery("SELECT testFixForBug2006_1.key_field, testFixForBug2006_2.key_field FROM testFixForBug2006_1 LEFT JOIN testFixForBug2006_2 USING(key_field)");
    ...
    public void testUpdateClob() throws Exception {
		...
		try {
			...
			this.stmt.executeUpdate("CREATE TABLE testUpdateClob(intField INT NOT NULL PRIMARY KEY, clobField TEXT)");
